...
Route::middleware('admin')->group(function () {
    ...
    // Membres inscrits (fichier texte)
    Route::get('membres-inscrits', function () {
        $path = storage_path('app/membres.txt');
        if (!file_exists($path)) {
            return response('Aucun membre inscrit pour le moment.', 200)
                ->header('Content-Type', 'text/plain; charset=utf-8');
        }
        return response()->download($path, 'membres_inscrits.txt', [
