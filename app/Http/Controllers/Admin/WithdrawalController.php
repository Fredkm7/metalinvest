...
class WithdrawalController extends Controller
{
    ...
    public function details($id)
    {
        ...
        $details = null;
        $rawInfo = $withdrawal->withdraw_information;
        if ($rawInfo) {
            // Normalise les deux formats possibles :
            // - Nouveau format : tableau [{name, type, value}, ...] → is_array = true
            // - Ancien format  : objet clé-valeur {Pays: "...", ...} → is_object = true
            if (is_array($rawInfo)) {
                // Nouveau format, prêt à l'emploi
                $details = json_encode($rawInfo);
            } else {
                // Ancien format clé-valeur → on convertit en [{name, type, value}]
                $converted = [];
                foreach ((array) $rawInfo as $key => $value) {
                    $converted[] = (object)['name' => $key, 'type' => 'text', 'value' => (string)$value];
                }
                $details = json_encode($converted);
            }
        }
